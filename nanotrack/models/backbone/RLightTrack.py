...
class MobileOne(nn.Module):
    ...
    """
    def __init__(self,
                 num_blocks_per_stage: List[int] = [1, 6, 7, 16],
        ...
        self.stage1 = self._make_1x1_stage(16, use_act=False, use_se=False)
        self.stage1 = self._make_3x3_stage(16, use_act=True, stride=2, use_se=False)
        self.stage1 = self._make_1x1_stage(16, use_act=False, use_se=False)
        """
        self.stage0 = MobileOneBlock(in_channels=3, out_channels=self.in_planes,
                                     kernel_size=3, stride=2, padding=0, inference_mode=self.inference_mode)
        self.stage1 = self._make_stage(int(32 * width_multipliers[0]), num_blocks_per_stage[0],
                                       num_se_blocks=0, stride=False, use_act=True)
        self.cur_layer_idx = 1
        self.stage2 = self._make_stage(int(64 * width_multipliers[1]), num_blocks_per_stage[1],
                                       num_se_blocks=0, stride=True, use_act=True)
        self.stage3 = self._make_stage(int(128 * width_multipliers[2]), num_blocks_per_stage[2],
                                       num_se_blocks=int(num_blocks_per_stage[2] // 2) if use_se else 0, stride=True, use_act=True)
        self.stage4 = self._make_stage(int(256 * width_multipliers[3]), num_blocks_per_stage[3],
                                       num_se_blocks=num_blocks_per_stage[3] if use_se else 0, stride=True, use_act=False)


    def _make_3x3_stage(self,
                    planes: int,
                    use_act: bool,
                    stride: int,
                    use_se: bool,
    ...
    def _make_1x1_stage(self,
                    planes: int,
                    use_act: bool,
                    use_se: bool,) -> nn.Sequential:
        """ Build a stage of MobileOne model.
    ...
    def _make_stage(self,
        ...
        for ix, stride in enumerate(strides):
            ...
            block_use_act = True
            if ix == num_blocks-1 and use_act is False:
                block_use_act = False
...
def mobileone(inference_mode: bool = False, variant: str = "s0") -> nn.Module:
    """Get MobileOne model.
    ...
    return MobileOne(inference_mode=inference_mode, **variant_params)
