...
class PixelwiseXCorr(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3):
        ...
        self.conv_kernel = nn.Sequential(
            # # dw
            nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, groups=in_channels, bias=False, padding=1),
            # nn.BatchNorm2d(out_channels),
            # nn.ReLU6(inplace=True),
            # pw
        ...
        self.adj_layer = nn.Conv2d(in_channels=64, out_channels=out_channels, kernel_size=1, stride=1)

        for modules in [self.CA_layer, self.adj_layer, self.conv_kernel, self.conv_search]:
            for m in modules.modules():
                if isinstance(m, nn.Conv2d):
                    n = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
                    m.weight.data.normal_(0, math.sqrt(2. / n))
...
class DepthwiseBAN(BAN):
    def __init__(self, in_channels=64, out_channels=64, weighted=False):
        ...
        # NanoTrackV2
        self.corr_pw_reg = PixelwiseXCorr(256, 128)

        self.corr_pw_cls = PixelwiseXCorr(256, 128)
        ...
        # ------------------------------------------------------cls-----------------------------------------------------#
        # layer0
        cls_tower.append(SeparableConv2d_BNReLU(in_channels=128, out_channels=256, kernel_size=5, padding=2))

        # layer1
        cls_tower.append(SeparableConv2d_BNReLU(in_channels=256, out_channels=256, kernel_size=5, padding=2))

        # layer2
        cls_tower.append(SeparableConv2d_BNReLU(in_channels=256, out_channels=256, kernel_size=3, padding=1))
        ...
        # ------------------------------------------- bbox --------------------------------------------------#
        # layer0
        bbox_tower.append(SeparableConv2d_BNReLU(in_channels=128, out_channels=192, kernel_size=3, padding=1))

        # layer1
        bbox_tower.append(SeparableConv2d_BNReLU(in_channels=192, out_channels=192, kernel_size=3, padding=1))
        ...
        # layer5
        bbox_tower.append(SeparableConv2d_BNReLU(in_channels=192, out_channels=192, kernel_size=5, padding=2))
        ...
        self.bbox_pred = nn.Sequential(
            nn.Conv2d(192, 4, kernel_size=3, stride=1, padding=1),
        )
    ...
    def forward(self, z_f, x_f):
        ...
        # bbox_reg = torch.sigmoid(bbox_reg)
        # bbox_reg = bbox_reg * 255
...
if __name__ == '__main__':
    ...
    z = torch.rand(1, 64, 8, 8)
    print(net(z, x)[0].shape, net(z, x)[1].shape, )
