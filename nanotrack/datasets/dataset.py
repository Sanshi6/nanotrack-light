...
class BANDataset(Dataset):
    def __init__(self, cfg):
        ...
        # create point target
        self.point_target = PointTarget(self.cfg)

        # create sub dataset
        self.all_dataset = []
        start = 0
        self.num = 0
        for name in self.cfg.DATASET.NAMES:
        ...
        videos_per_epoch = self.cfg.DATASET.VIDEOS_PER_EPOCH
        self.num = videos_per_epoch if videos_per_epoch > 0 else self.num
        self.num *= self.cfg.TRAIN.EPOCH
        self.pick = self.shuffle()
    ...
    def __getitem__(self, index):
        ...
        neg = self.cfg.DATASET.NEG and self.cfg.DATASET.NEG > np.random.random()
        # neg: negative sample
        # get one dataset
        if neg:
        ...
        # get labels
        cls, delta = self.point_target(bbox, self.cfg.TRAIN.OUTPUT_SIZE, neg)
