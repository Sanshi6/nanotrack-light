...
from nanotrack.models.model_builder import ModelBuilder, ModelBuilder_trt
...
def main():
    ...
    # create model
    model = ModelBuilder_trt(cfg)

    model.backbone_255 = backbone_255
    model.backbone_127 = backbone_127
    model.ban_head = head
        ...
        for v_idx, video in tqdm(enumerate(dataset)):
            ...
            if 'VOT2018-LT' == args.dataset:
                ...
                with open(result_path, 'w') as f:
                    for x in pred_bboxes:
                        f.write(','.join([str(i) for i in x]) + '\n')
                average_time = average_time + numpy.mean(track_times)
                time_path = os.path.join(model_path, 'time', '{}.txt'.format(video.name))
                with open(time_path, 'w') as f:
                    f.write("{:.6f}\n".format(numpy.mean(track_times)))
                    for x in track_times:
                        f.write("{:.6f}\n".format(x))
