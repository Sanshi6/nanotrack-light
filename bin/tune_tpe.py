...
from lib.models.siamrpn import SiamRPN_
...
parser.add_argument('--arch', dest='arch', default='OceanT',
                    help='architecture of model')
parser.add_argument('--resume', default=r'E:\SiamProject\track\snapshot\checkpoint_e50.pth', type=str,
                    help='resumed model')
...
def fitness(config, reporter):
    ...
    model_config['hp']['window_influence'] = window_influence
    model_config['hp']['lr'] = scale_lr
    model_config['hp']['small_sz'] = 255
    model_config['hp']['big_sz'] = 255
    model_config['hp']['ratio'] = ratio
...
if __name__ == "__main__":
    ...
    ray.init(num_gpus=args.gpu_nums, num_cpus=args.gpu_nums * 8,  object_store_memory=80000000)
    ...
    if args.dataset.startswith('VOT'):
        ...
        algo = HyperOptSearch(params, metric="AUC", mode='max')  #
    else:
        raise ValueError("not support other dataset now")

    # tune.run_experiments(tune_spec, search_alg=algo, scheduler=scheduler)
    experiment_spec = Experiment("experiment", fitness, stop={"AUC": 0.8},
            config={
                "penalty_k": hp.quniform('penalty_k', 0.001, 0.2, 0.001),
                "scale_lr": hp.quniform('scale_lr', 0.3, 0.8, 0.001),
                "window_influence": hp.quniform('window_influence', 0.15, 0.65, 0.001),
                "small_sz": hp.choice("small_sz", [255]),
                "big_sz": hp.choice("big_sz", [255, 271, 287, 303, 319]),
                "ratio": hp.quniform('ratio', 0.7, 1, 0.01),
            },
            resources_per_trial={
                "cpu": 4,
...
"""
    ...
    params = {
        ...
        "ratio": tune.quniform(0.7, 1, 0.01),
    }
    ...
    tuner = tune.Tuner(fitness, param_space=params,
                       run_config=RunConfig(name="my_tune_run", storage_path=r".\result",),
                       tune_config=TuneConfig(num_samples=100, mode='max', metric='AUC',
